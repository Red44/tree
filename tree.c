...
/* Globals */
bool dflag, lflag, pflag, sflag, Fflag, aflag, fflag, uflag, gflag, mflag;
...
int main(int argc, char **argv) {
    ...
    aflag = dflag = fflag = lflag = pflag = sflag = Fflag = uflag = gflag = mflag = false;
            ...
            for (j = 1; argv[i][j]; j++) {
                switch (argv[i][j]) {
                    ...
                    case 'p':
                        ...
                        break;
                    case 'm':
                        mflag = (opt_toggle ? !mflag : true);
                        if (isdigit(argv[i][j + 1])) {
                            for (k = 0; (argv[i][j + 1 + k] != '\0') && (isdigit(argv[i][j + 1 + k])) &&
                                        (k < PATH_MAX - 1); k++) {
                            ...
                            if ((sLevel = argv[n++]) == NULL) {
                                fprintf(stderr, "tree: Missing argument to -m option.\n");
                                exit(1);
                            }
                        }
                        maxfiles = (int) strtoul(sLevel, NULL, 0) - 1;
                        if (maxfiles < 0) {
                    ...
                    case 'I':
                        ...
                        if (ipattern >= maxipattern - 1)
                            ipatterns = xrealloc(ipatterns, sizeof(char *) * (size_t) (maxipattern += 10));
...
void usage(int n) {
    ...
    fancy(n < 2 ? stderr : stdout,
          "usage: \btree\r [\b-acdfmghilnpqrstuvxACDFJQNSUX\r] [\b-L\r \flevel\r [\b-R\r]] [\b-H\r [-]\fbaseHREF\r]\n"
    ...
    fancy(stdout,
          ...
          "  \b-m\r \ffiles\r      Limits the total files displayed per dir.\n"
    ...
    *n = -1;
    ...
    while ((ent = (struct dirent *) readdir(d))) {
        ...
        if (strlen(dir) + strlen(ent->d_name) + 2 > pathsize)
            path = xrealloc(path, pathsize = (strlen(dir) + strlen(ent->d_name) + PATH_MAX));
            ...
            if ((*dir)->lnk) {
                if (lflag) {
                    if (findino((*dir)->inode, (*dir)->dev)) {
                        ...
                        if (*(*dir)->lnk == '/')
                            (*dir)->child = unix_getfulltree((*dir)->lnk, lev + 1, dev, &((*dir)->size),
                                                             &((*dir)->err));
                        else {
            ...
            case '*':
                ...
                if (*pat == '*') {
                    ...
                    while (*buf && !(match = patmatch(buf, pat, isdir))) {
                        ...
                        if (pprev == '/' && *pat == '/' && *(pat + 1) && (match = patmatch(buf, pat + 1, isdir)))
                            return match;
                        buf++;
                        while (*buf && *buf != '/') buf++;
                    }
...
int psize(char *buf, off_t size) {
    ...
    if (hflag || siflag) {
        ...
        if (!idx) return sprintf(buf, " %4d", (int) size);
            ...
            return sprintf(buf, (((size + 52) / usize) >= 10) ? " %3.0f%c" : " %3.1f%c", (float) size / (float) usize,
                           unit[idx]);
